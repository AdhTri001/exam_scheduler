...
import type { RunScheduleParams, SuccessResponse, VersionInfo, ScheduleResponse } from './lib/wasmTypes';
...
function App() {
  ...
  const isStep2Complete = isStep1Complete && appState.params?.examStartDate && appState.params?.examEndDate;
...
function App() {
  ...
  const handleNext = () => {
    const nextStep = activeStep + 1;
    setActiveStep(nextStep);
    saveState({ ...appState, currentStep: nextStep });
  };

  const handleBack = () => {
    const prevStep = activeStep - 1;
    setActiveStep(prevStep);
    saveState({ ...appState, currentStep: prevStep });
  };

  const getStepContent = (step: number) => {
    switch (step) {
      case 0:
        return <Typography>Step 1: File Uploads</Typography>;
      case 1:
        return <Typography>Step 2: Configuration</Typography>;
      case 2:
        return <Typography>Step 3: Generation and Results</Typography>;
      default:
        return <Typography>Unknown Step</Typography>;
    }
  };
